...
#[Controller(prefix: 'parcels/transport')]
class ParcelTransportController extends ParcelBaseController
{
    #[Inject]
    ...
    public function bl(RequestInterface $request)
    {
        ...
        $painter         = $blSerice->BlNodeTransportLists(member: $member, vData: $vData);
        $painter['code'] = 200;
        $painter['msg']  = '加载完成';
    ...
    public function parcel(RequestInterface $request)
    {
        ...
        $painter['code'] = 200;
        $painter['msg']  = '加载完成';
        $painter['data'] = $parcelService->parcelNodeListAnalyse($member, $vData, 'transport');
        return $this->response->json($painter);
    }
    ...
    public function done(RequestInterface $request)
    {
        ...
        switch ($this->request->UserInfo['role_id']) {
            ...
            case 2:
                ...
                $result    = $blService->NodeDone(blSn: $doneData['bl_sn'], op_member_uid: $member['uid'], node: 'transport');
                break;
        }
    ...
    {
        $exceptionCode = [15201, 15203]; //config model =15200
        ...
        $member         = $request->UserInfo;
        $parcelService  = \Hyperf\Support\make(ParcelService::class);
        $result         = $parcelService->bindException(params: $params, member: $member, node: 'transport', code: $exceptionCode);
        return $this->response->json($result);
    }
    ...
    public function removeException(RequestInterface $request)
    {
        ...
        $result['msg']  = '解除异常失败';
        $parcelService  = \Hyperf\Support\make(ParcelService::class);
        $bool           = $parcelService->removeException(params: $params, member: $member, node: 'transport', code: $exceptionCode);
        if ($bool) {
            $result['code'] = 200;
            $result['msg']  = '解除异常成功';
        }
