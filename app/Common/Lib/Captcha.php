...
class Captcha
{
    ...
    {
        if (empty($config)) {
            $config = $this->configInterface->get('captcha');
        }
    ...
    protected function generate(): array
    {
        ...
        if ($this->math) {
            ...
            $and             = $x + $y;
            $keyCode['code'] = $and;
            $keyCode['time'] = time() + $this->expire;
            $key             = $this->crypt->encrypt(json_encode($keyCode));
     ...
     * @return bool 验证码是否正确
     */
    public function check(string $verify_id, string $verify_code): bool
    {
        $keyCode = json_decode($this->crypt->decrypt($verify_id), true);
        if (!isset($keyCode['time']) || $keyCode['time'] < time() || ($verify_code != $keyCode['code'])) {
            return false;
        }
        return true;
    }
    ...
    public function create(string $config = null, bool $api = false)
    {
        ...
        // 验证码使用随机字体
        $ttfPath = BASE_PATH . '/app/Common/Lib/Captcha/assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        foreach ($text as $index => $char) {
            ...
            $x     = $this->fontSize * ($index + 1) * mt_rand(1, 1) * ($this->math ? 1 : 1.5);
    ...
    protected function writeCurve(): void
    {
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->im, round($px + $i), round($py + $i), $this->color); // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                $i  = (int)($this->fontSize / 5);
                while ($i > 0) {
                    imagesetpixel($this->im, round($px + $i), round($py + $i), $this->color);
                    $i--;
                }
            }
        }
