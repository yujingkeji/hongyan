...
class SM4
{
    ...
    /**
     * sm4加密（ecb）
     * @param $secretKey EMS私钥
     * @param $data 明文数据
     * @return string 加密结果base64后加上|$4|头
     */
    public function encrypt($secretKey, $data)
    {
        ...
        return "|$4|".base64_encode($ciphertext);
    }

    /**
     * sm4解密
     * @param $secretKey EMS私钥
     * @param $data 密文数据(含|$4|头)
     * @return bool|string
     */
    public function decrypt($secretKey, $data)
    {
        $key = base64_decode($secretKey);
        $data = base64_decode(ltrim($data, "|$4|"));
